...
export const inverse = (_in, out = new Buffer(MatrixBufferLength)) => {
    if (Object.is(out, _in))
        _in = Buffer.from(_in);
    out[0] = _in[0];
    out[1] = _in[4];
    out[2] = _in[8];
...
export const transpose = (_in, out = new Buffer(MatrixBufferLength)) => {
    [
...
export const isIdentity = (m) => m[0] === 1 &&
    m[5] === 1 &&
    m[10] === 1 &&
    m[15] === 1 &&
    m[1] === 0 &&
    ...
    m[13] === 0 &&
    m[14] === 0;
export const matMatMul = (lhs, rhs, out = new Buffer(MatrixBufferLength)) => {
...
export const translation = (x, y = 0, z = 0, reset = false, out = new Buffer(MatrixBufferLength)) => {
    if (x === undefined)
        return identity(out);
...
export const rotationAroundX = (angle, reset = true, out = new Buffer(MatrixBufferLength)) => {
    setSinCos(angle);
    if (reset)
        identity(out);
    out[5] = out[10] = cos;
...
export const rotationAroundY = (angle, reset = true, out = new Buffer(MatrixBufferLength)) => {
    setSinCos(angle);
    if (reset)
        identity(out);
    out[0] = out[10] = cos;
...
export const rotationAroundZ = (angle, reset = true, out = new Buffer(MatrixBufferLength)) => {
    setSinCos(angle);
    if (reset)
        identity(out);
    out[0] = out[5] = cos;
