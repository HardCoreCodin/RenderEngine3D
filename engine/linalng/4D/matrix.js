...
import { equals, isIdentity, identity, inverse, transpose, matMatMul, translation, rotationAroundX, rotationAroundY, rotationAroundZ, } from "./arithmatic/matrix.js";
import { Buffer, VectorBufferLength, MatrixBufferLength, M0_Start, M1_Start, M2_Start, M3_Start, M0_End, M1_End, M2_End, M3_End, } from "./arithmatic/constants.js";
export default class Matrix4x4 {
    ...
    get isIdentity() { return isIdentity(this.buffer); }
    get inverted() {
        return new Matrix4x4(inverse(this.buffer));
    }
    inverse(inverted = Matrix4x4.Identity()) {
    ...
    times(rhs, new_matrix = new Matrix4x4()) {
    ...
    setTranslation(x = 0, y = 0, z = 0, reset = false) {
    ...
    setTo(x0, y0, z0, w0, x1, y1, z1, w1, x2, y2, z2, w2, x3, y3, z3, w3) {
        ...
        if (x0 instanceof Position4D || x0 instanceof Direction4D) {
            ...
            if (y0 instanceof Position4D || y0 instanceof Direction4D)
                this.m1.set(y0.buffer);
            if (z0 instanceof Position4D || z0 instanceof Direction4D)
                this.m2.set(z0.buffer);
            if (w0 instanceof Position4D || w0 instanceof Direction4D)
                this.m3.set(w0.buffer);
            return this;
        }
        if (typeof x0 === 'number') {
            this.buffer[0] = x0;
...
export const mat4 = (x0 = 1, y0, z0, w0, x1, y1 = 1, z1, w1, x2, y2, z2 = 1, w2, x3, y3, z3, w3 = 1) => new Matrix4x4().setTo(x0, y0, z0, w0, x1, y1, z1, w1, x2, y2, z2, w2, x3, y3, z3, w3);
