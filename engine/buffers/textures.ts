...
export class TextureMip extends Buffer<Float32Array>
{
    protected  _getAllocator() {return MATRIX_4X4_ALLOCATOR}

    public lines: Float32Array[][] = [];
    public colors = new Colors4D();
                ...
                for (let i = 0; i < 4; i++, tl++, tr++, bl++, br++) {
                    ...
                    if (wrap) {
                        ...
                        if (T) {
                            ...
                            if (     L) TL[tl] = component;
                            else if (R) TR[tr] = component;

                        } else if (B) {
                            BL[br] = BR[bl] = component;

                            if (     L) BL[bl] = component;
                            else if (R) BR[br] = component;
                        }
                    }
                }
            }
    ...
    sample(UV: UV2D, color: Color4D, filter: boolean = true) {
        let u = UV.array[0];
        let v = UV.array[1];
        if (u > 1) u -= u << 0;
        if (v > 1) v -= v << 0;
        ...
        const U = u * this.width  + offset; const x = U << 0;
        const V = v * this.height + offset; const y = V << 0;
        if (!filter) {
            color.array.set(this.colors.arrays[this.width * y + x]);
            return;
        }
        const r = U - x; const l = 1 - r;
        const b = V - y; const t = 1 - b;

        const tl = t * l;  const tr = t * r;
        const bl = b * l;  const br = b * r;
...
export class Texture {
    ...
    constructor(
        ...
        public width: number = image.width,
        public height: number = image.height
    ) {
    ...
    load(wrap: boolean = this.wrap, mipmap: boolean = this.mipmap): this {
        this.wrap = wrap;
        this.mips[0].load(wrap);
        this.mipmap = mipmap;
        if (mipmap) {
            let current_mip = this.mips[0];
            let next_mip: TextureMip;
            let mip_width  = this.width;
            let mip_height = this.height;

            let color, colors_quad: Float32Array;
            ...
            while (mip_width > 4 && mip_height > 4) {
                ...
                mip_width  /= 2; mip_width <<= 0;
                mip_height /= 2; mip_height <<= 0;
    ...
    sample(UV: UV2D, dUV: UV2D, color: Color4D, filter: boolean = this.filter) {
        let mip_level = 0;
        if (this.mipmap) {
