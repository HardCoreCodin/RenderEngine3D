...
export class TextureMip extends Buffer {
    ...
    load(wrap) {
        ...
        let pixel_index = 0;
        let component, tl, tr, bl, br;
        let L, R, T, B;
        let TL, TR, BL, BR, components;
        this.lines.length = 0;
        ...
        for (let y = 0; y < this.height; y++) {
            ...
            for (let x = 0; x < this.width; x++) {
                ...
                for (let i = 0; i < 4; i++, tl++, tr++, bl++, br++) {
                    ...
                    if (wrap) {
                        if (L)
                            current_line[r][br] = next_line[r][tr] = component;
                        else if (R)
                            current_line[l][bl] = next_line[l][tl] = component;
                        if (T) {
                            Bline[x][br] = Bline[x + 1][bl] = component;
                            if (L)
                                Bline[r][br] = component;
                            else if (R)
                                Bline[l][bl] = component;
                        }
                        else if (B) {
                            Tline[x][tr] = Tline[x + 1][tl] = component;
                            if (L)
                                Tline[r][tr] = component;
                            else if (R)
                                Tline[l][tl] = component;
                        }
                    }
                    else {
                        if (T) {
