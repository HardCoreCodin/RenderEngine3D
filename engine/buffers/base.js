export class BaseBuffer {
    constructor(count, stride, array, sub_arrays) {
        this.count = count;
        this.stride = stride;
        this.array = array;
        this.sub_arrays = sub_arrays;
        for (let i = 0; i < this.count; i++)
            this.sub_arrays[i] = this.array.subarray(this.stride * i, this.stride * (i + 1));
    }
}
export class Float32Buffer extends BaseBuffer {
    constructor(count, stride, array = new Float32Array(count * stride), sub_arrays = new Array(count)) {
        super(count, stride, array, sub_arrays);
        this.count = count;
        this.stride = stride;
        this.array = array;
        this.sub_arrays = sub_arrays;
        this.setTo = (values) => this.array.set(values);
    }
}
export class Uint32Buffer extends BaseBuffer {
    constructor(count, stride, array = new Uint32Array(count * stride), sub_arrays = new Array(count)) {
        super(count, stride, array, sub_arrays);
        this.count = count;
        this.stride = stride;
        this.array = array;
...
export class Uint8Buffer extends BaseBuffer {
    constructor(count, stride, array = new Uint8Array(count * stride), sub_arrays = new Array(count)) {
        super(count, stride, array, sub_arrays);
        this.count = count;
        this.stride = stride;
        this.array = array;
