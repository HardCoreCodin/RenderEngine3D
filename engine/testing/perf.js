...
// };
const add_array_of_arrays = (a_id, a_arrays, b_id, b_arrays, o_id, o_arrays) => {
...
//         add_object(a, b, o);
// };
const vec4_addition_passed_as_array_of_arrays = (a_id, a_arrays, b_id, b_arrays, o_id, o_arrays) => {
    for (let n = 0; n < num_iterations; n++)
        add_array_of_arrays(a_id, a_arrays, b_id, b_arrays, o_id, o_arrays);
};
...
function add_SoA(a_id, a_X, a_Y, a_Z, a_W, b_id, b_X, b_Y, b_Z, b_W, o_id, o_X, o_Y, o_Z, o_W) {
    o_X[o_id] = a_X[b_id] + b_X[b_id];
    o_Y[o_id] = b_Y[b_id] + b_Y[b_id];
    o_Z[o_id] = b_Z[b_id] + b_Z[b_id];
    o_W[o_id] = a_W[b_id] + b_W[b_id];
}
function SoA(a_id, aX, aY, aZ, aW, b_id, bX, bY, bZ, bW, o_id, oX, oY, oZ, oW) {
    for (let n = 0; n < num_iterations; n++)
        add_SoA(a_id, aX, aY, aZ, aW, b_id, bX, bY, bZ, bW, o_id, oX, oY, oZ, oW);
}
now = performance.now();
for (i = 0; i < arrays_count; i++)
    SoA(i, a_x, a_y, a_z, a_w, i, b_x, b_y, b_z, b_w, i, o_x, o_y, o_z, o_w);
console.log(`Round 1: add_SoA: ${performance.now() - now}`);
function add_AoS(a, b, o) {
    o[0] = a[0] + b[0];
    o[1] = a[1] + b[1];
    o[2] = a[2] + b[2];
    o[3] = a[3] + b[3];
