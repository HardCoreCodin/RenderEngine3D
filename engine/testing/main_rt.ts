...
import {pos2} from "../lib/accessors/position.js";
...
globalThis.setABC = (Ax: number, Ay: number, Bx: number, By: number, Cx: number, Cy: number): void => {
...
globalThis.getQt = (): number => (Q.x - A.x) / (B.x - A.x);
globalThis.getRt = (): number => (R.x - B.x) / (C.x - B.x);
globalThis.getPt = (): number => (P.x - Q.x) / (R.x - Q.x);

globalThis.getQ = (t: number): [number, number] => {A.lerp(B, t, Q); return [Q.x, Q.y]};
globalThis.getR = (t: number): [number, number] => {B.lerp(C, t, R); return [R.x, R.y]};
globalThis.getP = (t: number): [number, number] => {Q.lerp(R, t, P); return [P.x, P.y]};

globalThis.getPbyT = (t: number): [number, number] => {
    globalThis.getQ(t);
    globalThis.getR(t);
