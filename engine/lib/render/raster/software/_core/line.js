...
const drawDiagonalLine2D = (pixels, screen_width, screen_height, r, g, b, a) => {
...
const drawShallowLine2D = (pixels, screen_width, screen_height, r, g, b, a) => {
    // Shallow slope line:
...
const drawSteepLine2D = (pixels, screen_width, screen_height, r, g, b, a) => {
    // Steep slope line:
...
const drawVerticalLine2D = (pixels, screen_width, screen_height, r, g, b, a) => {
    // Vertical line
    start = current = y;
    end = y_end;
    if (rise > 0) {
...
const drawHorizontalLine2D = (pixels, screen_width, screen_height, r, g, b, a) => {
    // Horizontal line
    start = current = x;
    end = x_end;
    if (run > 0) {
...
export const drawLine2D = (pixels, screen_width, screen_height, x1, y1, x2, y2, r, g, b, a) => {
...
export const drawLine3D = (pixels, depths, screen_width, screen_height, x1, y1, z1, x2, y2, z2, r, g, b) => {
    // Optimized Bresenham's algorithm.
    ...
    if (dx === dy) {
        ...
        threshold; // What the error is compared agains at each iteration of the loop,
        if (dx > dy) {
            // Shallow slope line:
            // X increases more often than Y so use X as primary and Y as secondary:
            start = p = x;
...
let start, end, current, inc, ci, dci, zi, dzi, x, x_end, y, y_end, d, dz, z, dx, dy, run, rise, depth;
let p, dp, // The primary value being incremented (X for shallow slopes, Y for steep ones).
dsci, // The color-index increment for secondary value (Y for shallow slopes, X for steep ones).
e, dpe, dse, // The error being tracked and it's primary and secondary increment values.
threshold; // What the error is compared agains at each iteration of the loop,
