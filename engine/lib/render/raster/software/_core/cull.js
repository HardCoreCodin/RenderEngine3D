...
export const cullVertices = (arrays, vertex_flags, vertex_count) => {
    ...
    let has_inside, has_near;
    has_inside = has_near = false;
    let directions, shared_directions, w, x, y, z;
...
export const cullFaces = (vertex_positions, face_vertices_arrays, face_flags, vertex_flags, check_for_clipping = false, cull_back_faces = false, pz) => {
    // Check face intersections against the frustum:
    face_flags.fill(INSIDE);
    let has_inside, has_near;
    has_inside = has_near = false;
    let v1_index, v1_flags, x1, y1, z1, d1_x, d1_y, d1_z, v2_index, v2_flags, x2, y2, z2, d2_x, d2_y, d2_z, v3_index, v3_flags, x3, y3, z3, nx, ny, nz, dot;
    let v1, v2, v3;
    let f = 0;
