...
import { FacePositions3D, FacePositions4D, VertexPositions3D, VertexPositions4D } from "../buffers/attributes/positions.js";
...
export default class Faces {
    init(indices, include, position_dim, normal_dim, color_dim) {
        switch (position_dim) {
            case 3 /* _3D */:
                this.positions = new FacePositions3D().autoInit(indices);
                break;
            case 4 /* _4D */:
                this.positions = new FacePositions4D().autoInit(indices);
                break;
        ...
        if (include & 2 /* normal */) {
            switch (normal_dim) {
                case 3 /* _3D */:
                    this.normals = new FaceNormals3D().autoInit(indices);
                    break;
                case 4 /* _4D */:
                    this.normals = new FaceNormals4D().autoInit(indices);
                    break;
        ...
        if (include & 4 /* color */) {
            switch (color_dim) {
                case 3 /* _3D */:
                    this.colors = new FaceColors3D().autoInit(indices);
                    break;
                case 4 /* _4D */:
                    this.colors = new FaceColors4D().autoInit(indices);
                    break;
                default: throw `Invalid dimension for face colors: Expected 3D/4D got ${color_dim}!`;
            }
        }
    }
    pullNormalsFrom(vertex_potitions) {
    ...
    mul(matrix, out) {
        if (matrix instanceof Matrix3x3) {
            ...
            if (this.positions instanceof FacePositions3D)
                this.positions.mul(matrix, out.positions);
            else if (this.positions instanceof FacePositions4D &&
                out.positions instanceof FacePositions4D)
                this.positions.mul(matrix, out.positions);
            if (this.normals instanceof FaceNormals3D)
                this.normals.mul(matrix, out.normals);
            else if (this.normals instanceof FaceNormals4D &&
