...
export const shadeFace1 = (shader, pixel, surface, scene, depths, pixels, v1, v2, v3, v1_position, v2_position, v3_position, v1_normal, v2_normal, v3_normal, v1_uv, v2_uv, v3_uv, has_normals, has_uvs) => {
    let pixel_depth, z;
    ...
    for (let y = min_y; y <= max_y; y++, C_start += Cdy, B_start += Bdy, pixel_start += screen_width) {
        ...
        C += Cdx * x0;
        last_UV_taken = false;
        for (let x = x0; x <= x1; x++, B += Bdx, C += Cdx, pixel_index++) {
            if (Bdx < 0 && B < 0 ||
                Cdx < 0 && C < 0)
                break;
            A = 1 - B - C;
            ...
            Cw3 = C * w3;
            z = 1.0 / (Aw1 + Bw2 + Cw3);
            pixel.perspective_corrected_barycentric_coords.A = Ap = Aw1 * z;
            pixel.perspective_corrected_barycentric_coords.B = Bp = Bw2 * z;
            pixel.perspective_corrected_barycentric_coords.C = Cp = Cw3 * z;
            pixel.coords.x = x;
            pixel.coords.y = y;
            pixel.depth = z;
