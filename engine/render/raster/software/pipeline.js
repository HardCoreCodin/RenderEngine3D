...
export default class Rasterizer extends BaseRenderPipeline {
    ...
    render(viewport) {
        ...
        let result, face_index, vertex_index, face_count, vertex_count;
        ...
        for (const material of this.scene.materials)
            if (material instanceof SoftwareRasterMaterial) {
                for (mesh of material.mesh_geometries.meshes) {
                    ...
                    const attributes = mesh_has_normals || mesh_has_uvs ? [] : null;
                    if (mesh_has_normals)
                        attributes.push([
                                    ...
                                    for (face_index = 0; face_index < face_count; face_index++, vertex_index += 3) {
                                        if (ff[face_index]) {
                                            ...
                                            if (viewport.show_wire_frame) {
                                                ...
                                                shadeFace(material.pixel_shader, inputs, this.depth_buffer, viewport.render_target.array, v1, v2, v3, mesh_has_normals ? normals : undefined, mesh_has_uvs ? uvs : undefined);
                                            }
                                        }
                                    }
                                }
