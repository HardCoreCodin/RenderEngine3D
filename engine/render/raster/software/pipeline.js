...
import { MAX_RENDER_TARGET_SIZE, NDC, NEAR, OUT } from "../../../core/constants.js";
...
export default class Rasterizer extends BaseRenderPipeline {
    constructor(context, scene) {
        ...
        this.pixels = new Pixels().init(MAX_RENDER_TARGET_SIZE);
        this.vertex_flags = new FlagsBuffer1D().init(CUBE_VERTEX_COUNT);
        this.clipped_world_positions = new VertexPositions3D().init(6);
        this.clipped_vertex_positions = new VertexPositions4D().init(6);
        this.clipped_vertex_normals = new VertexPositions3D().init(6);
        this.clipped_vertex_uvs = new UVs2D().init(6);
        this._scratch_vectors3D = new VertexPositions3D().init(9);
        this._scratch_vectors2D = new UVs2D().init(2);
        this._shaded = {
            viewing_origin: null,
    ...
    render(viewport) {
        ...
        let indices;
        let v1_index, v1_flags, x1, y1, z1, new_v1x, new_v2x, in2x, nx, d1_x, d2_x, v2_index, v2_flags, x2, y2, z2, new_v1y, new_v2y, in2y, ny, d1_y, d2_y, v3_index, v3_flags, x3, y3, z3, new_v1z, new_v2z, in2z, nz, d1_z, d2_z, new_v2num, out1_index, out1_num, out1z, in1_index, in1_num, in1z, new_v1num, out2_index, out2_num, out2z, in2_index, in2_num, dot, t, one_minus_t, clipped_index, component_count, one_over_length, face_index, vertex_index, face_count, vertex_count;
        ...
        let mesh_has_normals, mesh_has_uvs, clipping_produced_an_extra_face;
        for (const material of this.scene.materials)
            if (material instanceof SoftwareRasterMaterial) {
                ...
                for (mesh of material.mesh_geometries.meshes) {
                    ...
                    indices = mesh.face_vertices.arrays;
                    const attributes = [[world_positions.arrays, indices, positions, false]];
                    if (mesh_has_normals)
                        attributes.push([world_normals.arrays, indices, normals, true]);
                    if (mesh_has_uvs)
                        attributes.push([mesh.vertices.uvs.arrays, indices, uvs, false]);
                            ...
                            if (material.mesh_shader(this._mesh_shader_inputs, this._mesh_shader_outputs)) {
                                ...
                                if (cullVertices(vertex_positions, vf, vertex_count)) {
                                    ...
                                    for (face_index = 0; face_index < face_count; face_index++) {
                                        ...
                                        face_vertex_indices = mesh.face_vertices.arrays[face_index];
                                        ...
                                        if ((v1_flags | v2_flags) | v3_flags) {
                                            ...
                                            if (v1_flags & NEAR ||
                                                ...
                                                in1 = vertex_positions[in1_index];
                                                in1z = in1[2];
                                                out1 = vertex_positions[out1_index];
                                                out1z = out1[2];
                                        ...
                                        for (vertex_index = 0; vertex_index < vertex_count; vertex_index++, v1_index += 3, v2_index += 3, v3_index += 3) {
                                            ...
                                            if (viewport.show_wire_frame) {
                                                ...
                                                shadeFace(material.pixel_shader, this._shaded, this.pixels, viewport.size, v1, v2, v3, positions[v1_index], positions[v2_index], positions[v3_index], normals[v1_index], normals[v2_index], normals[v3_index], uvs[v1_index], uvs[v2_index], uvs[v3_index], mesh_has_normals, mesh_has_uvs);
                                            }
                                        }
                                    }
                                }
